...
static void vidqueue_poll(struct vtx *vtx, uint64_t jfs, uint64_t prev_jfs)
{
	...
	pthread_mutex_lock(&vtx->mutex_tx);
 ...
 out:
	pthread_mutex_unlock(&vtx->mutex_tx);
}
...
static void video_destructor(void *arg)
{
	...
	pthread_mutex_destroy(&vtx->mutex_tx);
...
static int vtx_alloc(struct vtx *vtx, struct video *video)
{
	...
	err = pthread_mutex_init(&vtx->mutex_tx, NULL);
	if (err)
		return err;
	...
	tmr_start(&vtx->tmr_rtp, 0, rtp_tmr_handler, vtx);
